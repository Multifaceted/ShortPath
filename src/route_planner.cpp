...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for (RouteModel::Node* nptr:current_node->neighbors){
        ...
        nptr->g_value=current_node->g_value +  // ??????????????????????????????????
        nptr->visited=true;
...
RouteModel::Node *RoutePlanner::NextNode() {
    bool Compare(RouteModel::Node* n1, RouteModel::Node* n2) {
        int f1 = n1->h_value + n1->g_value; // f1 = g1 + h1
        int f2 = n2->h_value + n2->g_value; // f2 = g2 + h2
        return f1 > f2; 
}
    std::sort(open_list->begin(), open_list->end(), Compare);
    RouteModel::Node* small = open_list.back();
    open_list.pop_back();
    return small;
}
}
...
void RoutePlanner::AStarSearch() {
    RouteModel::Node *current_node = start_node; // modification ??????
